...
class ParamPosRequestDataMapperTest extends TestCase
{
     ...
     */
    public function testCreate3DEnrollmentCheckRequestData(array $order, string $paymentModel, string $txType, CreditCardInterface $card, array $expected): void
    {
        $requestDataWithoutHash = $expected;
        unset($requestDataWithoutHash['Islem_Hash']);

        $this->crypt->expects(self::once())
            ->method('generateRandomString')
            ->willReturn($expected['Islem_ID']);
        $this->crypt->expects(self::once())
            ->method('createHash')
            ->with($this->account, $requestDataWithoutHash)
            ->willReturn($expected['Islem_Hash']);
